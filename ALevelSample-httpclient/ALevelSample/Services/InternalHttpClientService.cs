...
public class InternalHttpClientService : IInternalHttpClientService
{
        ...
        where TRequest : class
    {
        ...
        {
            httpMessage.Content =
                new StringContent(JsonConvert.SerializeObject(content, new JsonSerializerSettings() { ContractResolver = new CamelCasePropertyNamesContractResolver() }), Encoding.UTF8, "application/json");
        }
        ...
        {
            var resultContent = await result.Content.ReadAsStringAsync();
            if (resultContent != null && !string.Equals(resultContent, string.Empty))
            {
                var response = JsonConvert.DeserializeObject<TResponse>(resultContent);
                return response;
            }
